var Fluid = (function(fluid){
	...
	function dealPart(from, to, arr){
		...
		for(; i< to ; i++){
			...
			threadArr.push(setTimeout(eval(config.showClass), _timeout*j, _clone, arr[i]));
			j++;
		}
	}
	
	/*从arr中处理第index部分的数据[from, to)*/
	function dealChain(index, arr){
		var len = arr.length;
		var part = Math.floor(len / _area) + 1;
		var from = (index-1)*_area,
			to = index*_area;
		if(index == part){
			to = len + 1;
		}
		...
		dealPart(from, to, arr);
		
		//最后一部分后，停止循环
		if(index != part){
			newThread = setTimeout(dealChain, _area*_timeout, ++index, arr);
		}
	}
